...
router.put("/comment", protectedRoute, (req, res) => {
    ...
    PostModel.findByIdAndUpdate(req.body.postId, {
        ...
        .populate("comments.commentedBy", "_id fullName")// post owner
        .exec((error, result) => {
            if (error) {
                return res.status(400).json({ error: error });
            } else {
